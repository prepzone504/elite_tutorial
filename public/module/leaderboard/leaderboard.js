...
(function () {
    ...
    async function initLeaderboardCore() {
        ...
        // ── FETCH BOTH MAPS IN PARALLEL ──
        await Promise.all([
            fetchSubjects(subjectSelect),
            fetchPuzzles(puzzleSelect)
        ]);

        // Default: show exam tab first
        currentTab = 'exam';
        if (examSelectWrapper) examSelectWrapper.style.display = 'block';
        if (puzzleSelectWrapper) puzzleSelectWrapper.style.display = 'none';
        if (tabs) {
            tabs.forEach(t => {
                if (t.dataset.tab === 'exam') t.classList.add('active');
        ...
        if (!initialized) {
            ...
            tabs.forEach(tab => {
                tab.addEventListener('click', (e) => {
                    ...
                    // Exam tab: show exam subjects, hide puzzle list
                    // Puzzle tab: hide exam subjects, show elite puzzles ONLY
    ...
    async function loadLeaderboard(podium1, podium2, podium3, lbList, stickyBar, usbRank, usbScore) {
        ...
        if (currentTab === 'exam') {
            ...
            // Old attempts may have null/Generic subject, resolved via batchSubjectMap
            ...
            if (error) {
                ...
                rawData = data.filter(row => {
                    // Resolve subject: use stored subject, or look up via batch ID map
